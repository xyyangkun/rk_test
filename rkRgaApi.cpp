...
int rkRgaBlit1(struct rkRgaCfg *src_cfg, struct rkRgaCfg *dst_cfg, struct rkRgaCfg *src1_cfg)
{
    ...
    rga_set_rect(&src.rect, 0, 0, src_cfg->width, src_cfg->height, src_cfg->width/*stride*/, src_cfg->height, src_cfg->fmt);
    rga_set_rect(&dst.rect, 0, 0, dst_cfg->width, dst_cfg->height, dst_cfg->width/*stride*/, dst_cfg->height, dst_cfg->fmt);
    rga_set_rect(&src1.rect, 0, 0, src1_cfg->width, src1_cfg->height, src1_cfg->width/*stride*/, src1_cfg->height, src1_cfg->fmt);
    ...
    /********** call rga_Interface **********/
    ret = rkRga.RkRgaBlit(&src, &dst, &src1);
    if (ret) {
	    printf("rgaFillColor error : %s\n", strerror(errno));
    }
